...
// Sağ alt köşede sabit WhatsApp chat widget'ı
...
function WhatsAppWidget() {
  ...
  // WhatsApp telefon numarası (environment variable'dan al)
  // Format: Ülke kodu ile birlikte, boşluksuz (örn: 905551234567)
  const whatsappNumber = import.meta.env?.VITE_WHATSAPP_NUMBER || '';
  
  // WhatsApp mesajı (önceden yazılmış)
  ...
  // WhatsApp numarası yoksa widget'ı gösterme
  if (!whatsappNumber) {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      {/* Chat Kutusu - Açıkken gösterilir - Daha küçük ve şeffaf */}
      {isOpen && (
        <div className="mb-3 bg-white/95 backdrop-blur-md rounded-2xl shadow-lg w-72 sm:w-80 max-w-[calc(100vw-5rem)] border border-gray-200/50 overflow-hidden transform transition-all duration-300 ease-out origin-bottom-right">
          ...
          <div className="bg-gradient-to-r from-green-500/90 to-green-600/90 backdrop-blur-sm text-white p-3 flex items-center justify-between relative">
            <div className="flex items-center gap-2.5 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="font-semibold text-sm truncate">Dr. Başak Şeref</h3>
          ...
          {/* Chat Body - Kompakt ve şeffaf */}
          <div className="p-3.5 bg-white/80 backdrop-blur-sm">
            {/* Mesaj balonu - Daha küçük */}
            <div className="mb-3">
              <div className="bg-white/90 rounded-xl rounded-tl-none p-3 shadow-sm border border-gray-100/50 relative backdrop-blur-sm">
                <p className="text-xs text-gray-800 leading-relaxed mb-1.5">
                  👋 Merhaba! Randevu almak veya sorularınız için WhatsApp'tan yazabilirsiniz.
                </p>
                {/* Balon kuyruğu */}
            ...
            {/* WhatsApp Butonu - Kompakt ve belirgin */}
            <a
              href={whatsappUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="group relative block w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white text-center py-2.5 px-4 rounded-xl font-semibold text-sm shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 transform hover:scale-[1.02] active:scale-[0.98]"
            >
              <svg 
                className="w-5 h-5" 
                fill="currentColor" 
      ...
      {/* WhatsApp Butonu - Her zaman görünür, sadece WhatsApp ikonu */}
      <div className="flex items-center gap-3 group">
        {/* Yanında görünen yazı - Mobilde gizli, desktop'ta görünür */}
        {!isOpen && (
          <div className="hidden md:block bg-white/90 backdrop-blur-sm text-gray-800 px-3 py-1.5 rounded-full shadow-md font-medium text-xs whitespace-nowrap border border-gray-200/50">
            💬 Randevu Al
          </div>
        )}
        ...
        <div className="relative">
          {/* Yumuşak pulse animasyonu - Sürekli büyüyüp küçülen halka */}
          {!isOpen && (
            <>
          ...
          {/* Ana buton - Her zaman WhatsApp ikonu, X olmaz */}
          <button
